...
@task
def dump_course_to_neo4j(course_key_string, options):
    """
    ...
    graph = authenticate_and_create_graph(options)
...
class ModuleStoreSerializer(object):
    ...
    def dump_courses_to_neo4j(self, options, override_cache=False):
        """
        ...
        for index, course_key in enumerate(self.course_keys):
            ...
            dump_course_to_neo4j.apply_async(
                args=[six.text_type(course_key), options],
...
def authenticate_and_create_graph(options):
    host = options['host']
